...
export default function ClientsSection() {
  ...
  async function inviteClient(e: React.FormEvent) {
    ...
    const res = await fetch("/api/clients", {
      ...
      body: JSON.stringify({ firstName: iFirst, lastName: iLast, email: iEmail, phone: iPhone, programId: iProg || null }),
    });
    setISaving(false);
    if (!res.ok) { const d = await res.json(); setIError(d.error ?? "Error"); return; }
    setIFirst(""); setILast(""); setIEmail(""); setIPhone(""); setIProg("");
    setShowInvite(false);
    fetchClients();
  }
    ...
    return (
      <div>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 24 }}>
          <div>
            <h1 style={{ fontSize: 22, fontWeight: 600, color: "#F0F0F0", margin: 0, marginBottom: 6 }}>
              {selected.firstName} {selected.lastName}
            </h1>
            <div style={{ color: "#A0A0A0", fontSize: 14 }}>{selected.email}</div>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <select
              ...
              style={{ ...inp, width: "auto", fontSize: 13, padding: "7px 12px" }}
            >
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24 }}>
        <h1 style={{ fontSize: 22, fontWeight: 400, color: "#F0F0F0", margin: 0 }}>Clients</h1>
          ...
          <form onSubmit={inviteClient}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 18 }}>
              ...
              <div>
                ...
                <select value={iProg} onChange={e => setIProg(e.target.value)} style={inp}>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead>
              <tr style={{ borderBottom: "1px solid #2A2A2A" }}>
                {["Name", "Email", "Program", "Status", "Last Active", ""].map(h => (
                  <th key={h} style={{
            ...
            <tbody>
              {clients.map((c, i) => (
                <tr key={c.id} style={{ borderBottom: i < clients.length - 1 ? "1px solid #1E1E1E" : "none" }}>
                  ...
                  <td style={{ padding: "14px 16px", color: "#A0A0A0", fontSize: 13 }}>{c.email}</td>
